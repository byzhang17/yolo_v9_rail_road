...
class DetectAPI:
    def __init__(self, weights='./runs/train/exp52/weights/best.pt', source='./datasets/rail/images/test', data='./data/rail.yaml',imgsz=[640, 640],
    ...
    def run(self, source, savedir = None):
        ...
        for path, im, im0s, vid_cap, s, progress in dataset:
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if cls == 5:
                            ...
                            threshold = 0.5 if cls == 25 else 0.35
                            if conf >= threshold:
                                txts.append([x1, y1, w, h, 5, conf])
                        elif cls == 6 and conf < 0.5:
                            continue
                        elif cls == 10:
                            txts.append([x1, y1, w, h, 37, conf])
                        elif cls == 23:
                            if w > 0.25 or conf < 0.3:
                                continue
                            txts.append([x1, y1, w, h, 36, conf])
                        elif cls == 30:
                            txts.append([x1, y1, w, h, 39, conf])
                        elif cls == 0:
                            crop = imc[:, x1:x2]
            ...
            if len(txts)>=0:
                ...
                txts = []
            # Print time (inference-only)
            LOGGER.info(f"{s}{'' if len(txts) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms")
            
            print(config.shared_data["stop"])
        ...
        if os.path.isdir(source):
            wpath = saveDir + '/result_model.json'
            f = open(wpath, 'w')
